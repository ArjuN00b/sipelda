...
<body>
    ...
    <div class="container">
        <div class="page-title">
            ...
            <p>Pantau rincian lengkap status dan tanggapan dari semua laporan yang pernah Anda kirimkan.</p>
        </div>
        ...
        <div class="history-cards">
            ...
            ?>
                    <div class="history-card" data-searchable="<?= htmlspecialchars(strtolower($kategori_murni . ' ' . $lokasi_detail . ' ' . $deskripsi_murni . ' ' . $row['id_pengaduan'])) ?>">
                        ...
                        <div class="detail-body-container">
                            <div class="detail-media">
                                <?php if ($row['foto']): ?>
                                    ...
                                    <?php endif; ?>
                                <?php else: ?>
                                    <div style="padding: 60px 20px; text-align: center; color: #94a3b8; font-size:15px;"><i class="fa-regular fa-image" style="font-size: 36px; display:block; margin-bottom:10px;"></i>(Tanpa Bukti Lampiran)</div>
                                <?php endif; ?>
                            </div>

                            <div class="detail-info-content">
                                <!-- GAMBAR PETA MAPS LEAFLET TERINTEGRASI DI ATAS DETAIL ALAMAT/LOKASI -->
                                <?php if ($card_lat && $card_lng): ?>
                                    <div id="card-map-<?= $row['id_pengaduan'] ?>" class="card-mini-map"></div>
                                <?php endif; ?>

                                <div class="box-lokasi">
                                    <i class="fa-solid fa-location-dot" style="color: #dc3545;"></i>
                                    <span><strong>Alamat / Detail Lokasi:</strong> <?= htmlspecialchars($lokasi_detail) ?></span>
                                </div>

                                <?php if ($link_maps): ?>
                                    <div class="box-koordinat">
                                        <i class="fa-solid fa-map-location-dot" style="color: #2563eb;"></i>
                                        <span><strong>Titik GPS & Peta:</strong> <a href="<?= htmlspecialchars($link_maps) ?>" target="_blank">Buka di Google Maps ↗</a></span>
                                    </div>
                                <?php endif; ?>
                                ...
                                <div class="tanggapan-box">
                                    <h4 style="margin:0 0 8px; font-size:15px; color:#002855;"><i class="fa-regular fa-comment-dots"></i> Tanggapan Resmi Kelurahan:</h4>
                                    <p style="margin:0; font-size:15px; font-style:<?= !empty($row['isi_tanggapan']) ? 'normal' : 'italic' ?>; color:#334155;">
                                        <?= !empty($row['isi_tanggapan']) ? htmlspecialchars($row['isi_tanggapan']) : 'Laporan Anda sedang dalam tahap evaluasi petugas.' ?>
                                    </p>
                                </div>
                            </div>
                        </div>
                ...
                <div style="text-align:center; padding: 60px; background:white; border-radius:16px; border:1px solid #e2e8f0; color:#64748b; font-size:16px;">
                    ...
                    Anda belum memiliki riwayat pengaduan yang sesuai.
                </div>
            <?php endif; ?>
        </div>
        ...
        <?php if ($total_pages > 1): ?>
            <div class="pagination-container">
                <a href="historipengaduan.php?page=<?= $page - 1 ?>" class="page-link <?= $page <= 1 ? 'disabled' : '' ?>"><i class="fa-solid fa-chevron-left"></i> SEBELUMNYA</a>
                
                <?php for ($i = 1; $i <= $total_pages; $i++): ?>
                    <a href="historipengaduan.php?page=<?= $i ?>" class="page-link <?= $i == $page ? 'active' : '' ?>"><?= $i ?></a>
                <?php endfor; ?>

                <a href="historipengaduan.php?page=<?= $page + 1 ?>" class="page-link <?= $page >= $total_pages ? 'disabled' : '' ?>">SELANJUTNYA <i class="fa-solid fa-chevron-right"></i></a>
            </div>
        <?php endif; ?>
    ...
    <script>
        ...
        // INISIALISASI PETA MINI PADA MASING-MASING CARD LAPORAN RIWAYAT
